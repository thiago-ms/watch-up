...
import br.com.shopper.watchup.core.data.model.REDES_CINEMA
import br.com.shopper.watchup.core.data.model.STREAMINGS_DISPONIVEIS
...
private fun PassoDetalhes(d: FormDraft, onChange: (FormDraft) -> Unit) {
    OutlinedTextField(
        value = d.genero,
        onValueChange = { onChange(d.copy(genero = it)) },
        label = { Text("Gênero") },
        singleLine = true,
        enabled = !d.semGenero,
        modifier = Modifier.fillMaxWidth(),
    ...
    Row(
        modifier = Modifier
            ...
            .toggleable(
                value = d.semGenero,
                role = Role.Checkbox,
                onValueChange = { onChange(d.copy(semGenero = it, genero = if (it) "" else d.genero)) },
            )
