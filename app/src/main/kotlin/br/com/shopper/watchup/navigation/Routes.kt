...
object Routes {
    ...
    const val REGISTRATION_NEW_ROUTE =
        "registration/new?titulo={titulo}&ano={ano}&tipo={tipo}&poster={poster}"
    ...
    fun registrationPrefill(titulo: String, ano: String?, tipoNome: String, posterUrl: String?): String {
        fun enc(s: String?) = Uri.encode(s.orEmpty())
        return "registration/new?titulo=${enc(titulo)}&ano=${enc(ano)}" +
            "&tipo=${enc(tipoNome)}&poster=${enc(posterUrl)}"
    }
}
